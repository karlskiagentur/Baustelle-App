...
const SEITEN = [
  { key: "heute", label: "Heute", ico: "🏗️", comp: Heute },
  { key: "material", label: "Material", ico: "🛒", comp: Material },
  { key: "karte", label: "Karte", ico: "🗺️", comp: Karte },
  { key: "zeit", label: "Zeitkonto", ico: "⏱️", comp: Zeitkonto },
  { key: "dokumente", label: "Dokumente", ico: "📄", comp: Dokumente },
  { key: "mehr", label: "Mehr", ico: "☰", comp: Mehr },
];
  ...
  return (
    <div className="app">
      <header className="kopf">
        <div>
          <h1>{unter === "urlaub" ? "Urlaub" : aktiv.label}</h1>
          <div className="sub">{s.name}{s.kolonne ? ` · ${s.kolonne}` : ""}</div>
        </div>
        {unter && <button className="btn hell klein-btn" onClick={() => setUnter(null)}>Zurück</button>}
      </header>
      ...
      <nav className="nav">
        {SEITEN.map((x) => (
          <button key={x.key} className={seite === x.key && !unter ? "aktiv" : ""} onClick={() => { setSeite(x.key); setUnter(null); }}>
            <span className="ico">{x.ico}</span>{x.label}
          </button>
        ))}
      </nav>
    </div>
